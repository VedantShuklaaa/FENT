...
use crate::{errors::FentError, events::{PtRedeemed, YieldClaimed}, state::{ActivityCounter, ActivityRecord, ActivityType, Market, Position}};
...
pub fn redeem_pt(ctx: Context<RedeemPt>, amount: u64) -> Result<()> {
    ...
    require!(ctx.accounts.market.maturity_ts <= now, FentError::NotMaturedYet);
    require!(ctx.accounts.user_pt_account.amount >= amount, FentError::InsufficientPt);
    ...
    ctx.accounts.position.pt_amount        = ctx.accounts.position.pt_amount.checked_sub(amount).ok_or(FentError::MathOverflow)?;
    ctx.accounts.position.last_activity_at = now;
    ctx.accounts.market.total_deposited    = ctx.accounts.market.total_deposited.checked_sub(amount).ok_or(FentError::MathOverflow)?;
...
pub fn claim_yield(ctx: Context<ClaimYield>) -> Result<()> {
    ...
    let yield_amount  = (yt_amount as u128).checked_mul(index_delta).ok_or(FentError::MathOverflow)?
                        .checked_div(1_000_000_000).ok_or(FentError::MathOverflow)? as u64;
    ...
    ctx.accounts.position.total_yield_claimed = ctx.accounts.position.total_yield_claimed.checked_add(yield_amount).ok_or(FentError::MathOverflow)?;
    ctx.accounts.position.last_activity_at = now;
